...
var UNINIT = true;
// 0. 先连接
var ktv_socket = io.connect('http://' + document.domain + ':' + location.port);


/* 以下是各个事件的监听函数
*/
// 1.监听 connect 事件，连结服务器，加入房间
ktv_socket.on('connect', function () {
    CLIENT_ID = ktv_socket.id;
    ktv_socket.emit('join', { 'room': ROOM_ID })
});
...
ktv_socket.on('initsongs', function (data) {
...
ktv_socket.on('message', function (data) {
    // 有人点歌
...
ktv_socket.on('order', function (data) {
...
ktv_socket.on('operatefail', function (msg) {
    // operation failure
    alert(msg);
});

// 4.监听 reportexist 事件，点名
ktv_socket.on('reportexist', function (msg) {
    // server call the roll
    if (msg == "1") {
        sendToServer_I_am_here();
    }
});

// 5.监听 adjust 事件，同步进度
ktv_socket.on('adjust', function (data) {
    // adjust offset of the video
    adjsutTo(data.starttime);
});
...
// 5.监听 getmessage 事件，收到消息
ktv_socket.on('getmessage', function (data) {
    // display message
    UI_recv_msg(data);
});
...
// -2. 发送 report 答到 指令给服务器，参数为 msg
function sendToServer_msg(msg) {
    ktv_socket.emit('sendmsg', {
        'username': USER,
        'room': ROOM_ID,
        'msg': msg
    });
...
// -1. 发送 report 答到 指令给服务器，参数为 room_id
function sendToServer_start(starttime) {
    ktv_socket.emit('starton', {
        'username': USER,
        'room': ROOM_ID,
        'starttime': starttime
    });
...
// 0. 发送 report 答到 指令给服务器，参数为 room_id
function sendToServer_I_am_here() {
    ktv_socket.emit('report', {
        'username': USER,
        'room': ROOM_ID
    });
}
